...
bushmeat <-  readRDS("./inputs/bushmeat_transects_phase3.rds") %>% 
  filter(village %ni% cut_villages) %>% 
  mutate_if(is.character,funs(factor(.)))  

effort <-  readRDS("./inputs/effort_transects_phase3.rds") %>% 
  filter(village %ni% cut_villages) %>% 
  mutate_if(is.character,funs(factor(.)))  
...
bushmeat %>% filter(especes %in% c("GHA", "BON", "BUF", "HYL", "FGA")) #BON confirmed real
bushmeat %>% filter(especes%in% c("COG", "PLQ")) #24 to change to CPE and PCM

bushmeat %>% filter(especes%in% c("CPE", "PCM")) #638

bushmeat <- bushmeat %>% 
  mutate(especes = case_when(especes == "COG" ~ "CPE",
                            especes == "PLQ" ~ "PCM",
                            especes %ni% c("COG", "PLQ") ~especes
                            ))

bushmeat %>% filter(especes%in% c("CPE", "PCM")) #662 (correct)

#phase 3,8 elephants hunted
...
# Create a list of modified tibbles with phase = 3
offtake_list_p3 <- tbl_names %>%
  set_names(paste0(tbl_names, "_p3")) %>%   # <- give new names with _p2
  map(~ {
...
saveRDS(offtake_list_p3, "./inputs/offtake/offtake_list_p3.rds")
...
offtake_yearly_species_p3 <- offtakeCMN %>% 
  dplyr::group_by(species) %>% 
  ...
  dplyr::mutate(phase = 3) %>% 
  dplyr::select(phase,everything())

saveRDS(offtake_yearly_species_p3, "./inputs/offtake/offtake_yearly_species_p3.rds")

#and by species by village

offtake_yearly_species_village_p3 <- offtakeCMN %>% 
...
saveRDS(offtake_yearly_species_village_p3, "./inputs/offtake/offtake_yearly_species_village_p3.rds")
